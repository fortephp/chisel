...
function getNthToken(
  tokens: { type: number; start: number; end: number }[],
  type: number,
  n: number,
) {
...
describe("JSX/TSX-Style Generics and Attributes", () => {
  ...
  describe("TypeScript Generics Edge Cases", () => {
    it("handles incomplete TypeScript generics at EOF", () => {
      ...
      const syntheticCloseCount = result.tokens.filter(
        (t) => t.type === TokenType.SyntheticClose,
      ).length;
    ...
    it("emits SyntheticClose before new tag in generic context", () => {
      ...
      const divIndex = tokens.findIndex(
        (t) => t.type === TokenType.TagName && tokenContent(template, t) === "div",
      );
    ...
    it("correctly handles attribute name followed by less-than", () => {
      ...
      const classIndex = tokens.findIndex(
        (t) => t.type === TokenType.AttributeName && tokenContent(template, t) === "class",
      );
