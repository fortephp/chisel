...
describe("Lexer Fault Tolerance", () => {
  describe("Incomplete Blade Delimiters", () => {
    it.each(["{{", "{{{", "{{--", "{!!", "{{ ", "{{ $", "{{ $x", "{!! ", "{!! $x"])(
      "handles incomplete echo start: %s",
      (input) => {
    ...
    it.each(["{{ $x !!}", "{!! $x }}", "{{{ $x }}", "{{ $x }}}"])(
      "handles mismatched echo delimiters: %s",
      (input) => {
        tokenizeAndVerify(input);
      },
    );
  ...
  describe("Incomplete Blade Comments", () => {
    it.each(["{{--", "{{-- comment", "{{-- comment --", "{{-- }} --", "{{-- {!! !!} --"])(
      "handles incomplete blade comments: %s",
      (input) => {
        tokenizeAndVerify(input);
      },
    );
  ...
  describe("Incomplete HTML Elements", () => {
    ...
    it.each(["</", "</d", "</div", "</div "])("handles incomplete closing tags: %s", (input) => {
      tokenizeAndVerify(input);
    });

    it.each(["<", ">", "< >", "> <", "<<", ">>", "<><", "><>"])(
      "handles orphan angle brackets: %s",
      (input) => {
        tokenizeAndVerify(input);
      },
    );
  ...
  describe("Incomplete HTML Comments", () => {
    ...
    it.each(["<!>", "<!x>", "<!DOCTYPE", "<!DOCTYPE ", "<!DOCTYPE html"])(
      "handles bogus comments: %s",
      (input) => {
        tokenizeAndVerify(input);
      },
    );
  ...
  describe("Incomplete Directives", () => {
    ...
    it.each(["@@", "@@if", "@@if($x)", "@@@if"])("handles escaped directives: %s", (input) => {
      tokenizeAndVerify(input);
    });
  });

  describe("CDATA Sections", () => {
    it.each(["<![CDATA[", "<![CDATA[content", "<![CDATA[content]", "<![CDATA[content]]"])(
      "handles incomplete CDATA: %s",
      (input) => {
        tokenizeAndVerify(input);
      },
    );

    it.each(["<![CDATA[content]]>", "<![CDATA[ <div> ]]>", "<![CDATA[ {{ $x }} ]]>"])(
      "handles complete CDATA: %s",
      (input) => {
        tokenizeAndVerify(input);
      },
    );
  ...
  describe("Generic Type Arguments (TSX)", () => {
    it.each(["<Map<", "<Map<Rec", "<Map<Rec>", "<Map<Rec>>", "<Map<<Rec", "<Map<<Rec<Rec<<>"])(
      "handles incomplete generic types: %s",
      (input) => {
        tokenizeAndVerify(input);
      },
    );
  ...
  describe("Script and Style Tags", () => {
    it.each(["<script>{{", "<script>{{ $x", "<script>{!! $x", "<script>@if($x)"])(
      "handles script with incomplete Blade: %s",
      (input) => {
        tokenizeAndVerify(input);
      },
    );
  ...
  describe("Quote Edge Cases", () => {
    ...
    it.each(['<div class="', "<div class='", '<div class="foo', "<div class='foo"])(
      "handles unclosed quotes: %s",
      (input) => {
        tokenizeAndVerify(input);
      },
    );
  ...
  describe("Whitespace Edge Cases", () => {
    ...
    it.each(["{{\n$x\n}}", "@if($x)\n@endif", '<div\nclass="foo"\n>', "<!--\ncomment\n-->"])(
      "handles whitespace in constructs: %j",
      (input) => {
        tokenizeAndVerify(input);
      },
    );
  ...
  describe("XML Declarations", () => {
    it.each(['<?xml version="1.0"?>', '<?xml version="1.0" encoding="UTF-8"?>', "<?xml", "<?xml "])(
      "handles XML declarations: %s",
      (input) => {
        tokenizeAndVerify(input);
      },
    );
  ...
  describe("Malformed Structures", () => {
    ...
    it.each(["@if @else @endif", "@if($x)@if($y)@endif", "{{ $x {{ $y }} }}", "@if(@if($x))"])(
      "handles malformed Blade: %s",
      (input) => {
        tokenizeAndVerify(input);
      },
    );
