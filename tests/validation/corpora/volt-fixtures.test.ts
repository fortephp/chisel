...
import { DEFAULT_OPTION_MATRIX, listRecursiveFixtureFiles } from "../support/fixture-suite.js";
import { defineCorpusFixtureSuite } from "../support/corpus-suite.js";

const VOLT_ROOT = join(process.cwd(), "tests", "fixtures", "validation", "volt");
const VOLT_FIXTURE_ROOT = join(VOLT_ROOT, "tests", "Feature", "resources", "views");
...
const FIXTURE_FILES = listRecursiveFixtureFiles(VOLT_FIXTURE_ROOT, (name) =>
...
defineCorpusFixtureSuite({
  ...
  defineExtraAssertions: () => {
    ...
    expect(
      FIXTURE_FILES.some((file) => file.startsWith("tests/Feature/resources/views/class-api/")),
    ).toBe(true);
    expect(FIXTURE_FILES.some((file) => file.includes("functional-api-pages/"))).toBe(true);
  },
});
